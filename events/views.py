...
def cad_edit(request, pk):
    jogador = get_object_or_404(Cadastro, pk=pk)
    if request.method == "POST":
        form = CadastroForm(request.POST, instance=jogador)
        if form.is_valid():
            post = form.save(commit=False)
            post.save()
            return redirect('cad_detail', pk=jogador.pk)
    else:
        form = CadastroForm(instance=jogador)
    context = {
        'jogador': Cadastro.objects.all(),
        'form': form,
    }
    return render(request, 'cad_edit.html', context)

def cad_list(request):
    jogadores = Cadastro.objects.all()
    return render(request, 'cad_list.html', {'jogadores': jogadores})


def cad_detail(request, pk):
    jogador = get_object_or_404(Cadastro, pk=pk)
    return render(request, 'cad_detail.html', {'jogador': jogador})
